...
import { Button, Card, Input, Select, Space, Tag, Alert } from 'antd';
import { DeleteOutlined } from '@ant-design/icons';
import { useQuery } from '@tanstack/react-query';
import { hallApi } from '@/api/hall';
import { queryKeys } from '@/api/queryKeys';
import WidgetRenderer from '@/components/device-catalog/WidgetRenderer';
import type { DeviceListItem, EffectiveCommand, DeviceCommandAction } from '@/api/gen/client';
...
interface Props {
  ...
  onChange: (patch: Partial<DeviceCommandAction>) => void;
  onRemove: () => void;
}

/**
 * 复用 SceneActionRow 模式：device → command → params 三选；改成 DeviceCommandAction 形态
 * （不绑定 scene，仅承载 device_command 卡的单条 action）。
 */
export default function DeviceCommandActionRow({ action, devices, onChange, onRemove }: Props) {
  ...
  const paramsValue = (action.params ?? {}) as Record<string, unknown>;
    ...
    >
      <Space wrap style={{ width: '100%' }}>
        <Select
          ...
          onChange={(v) =>
            onChange({ device_id: v as number, command: '', params: {} })
          }
        ...
        <div style={{ marginTop: 8 }}>
          {paramsKeys.length > 0 ? (
            ...
            <Input
              ...
              value={
                JSON.stringify(paramsValue) === '{}' ? '' : JSON.stringify(paramsValue)
              }
              onChange={(e) => {
                try {
