...
import type { ActionDef, RiskLevel, ScopeType, UserActionEntry } from '@/api/gen/client';
...
export function useDomainGroups(
  ...
  return useMemo(() => {
    ...
    // 1. 按 action_code 聚合 scope
    const byCode = new Map<string, Set<string>>();
    for (const e of safeEntries) {
      const key = `${e.scope.type}:${e.scope.id ?? ''}`;
      const set = byCode.get(e.action_code) ?? new Set<string>();
      set.add(key);
      byCode.set(e.action_code, set);
    }

    // 2. 按 domain 分组
    const byDomain = new Map<string, DomainGrantedAction[]>();
    for (const [code, scopeSet] of byCode) {
